...
export default function ExperienceComponents({ experience }: DataExp) {
  ...
  const getDisplayedText = (text: string, isExpanded: boolean) =>
    isExpanded || !shouldTruncate(text) ? text : text.slice(0, 100) + '...'
  return (
    <motion.div
    ...
    >
      <motion.h1
        className="font-bold text-4xl sm:text-5xl md:text-6xl xl:text-[96px]"
      ...
      >
        {experience.length === 0 ? (
          <p className="text-sm text-muted-foreground">No experience has been added yet</p>
        ) : (
